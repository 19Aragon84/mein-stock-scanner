...
min_market_cap_b = st.sidebar.number_input(
    "Min. Market Cap (Mrd. USD/EUR laut Yahoo-Wert)",
    value=10.0,
    step=1.0,
)

min_daily_dollar_vol_m = st.sidebar.number_input(
    "Min. Dollar-Volumen pro Tag (Mio.)",
    value=20.0,
    step=5.0,
)

min_div_yield = st.sidebar.number_input(
    "Min. Dividendenrendite für Wheel (%)",
    value=1.0,
    step=0.5,
)

max_dte = st.sidebar.slider(
    "Max. Laufzeit für CSP-Suche (Tage)",
    min_value=14,
    max_value=60,
    value=45,
)
...
min_oi = st.sidebar.number_input(
    "Min. Open Interest (Option)",
    value=100,
    step=50,
)

max_bid_ask_pct = st.sidebar.number_input(
    "Max. Bid-Ask-Spread (%)",
    value=8.0,
    step=1.0,
)
...
FALLBACK_TICKERS = {
    "S&P 500": [
        ...
        "UNH", "V", "MA", "COST", "AVGO", "HD", "PG", "JNJ", "ABBV", "BAC"
    ],
    "Nasdaq-100": [
        "AAPL", "MSFT", "NVDA", "AMZN", "META", "AVGO", "COST", "NFLX", "ADBE", "CSCO",
        "AMD", "QCOM", "AMGN", "INTU", "TXN", "ISRG", "BKNG", "ADI", "GILD", "AMAT"
    ],
    "Dow Jones": [
        "AAPL", "MSFT", "JPM", "GS", "HD", "MCD", "CAT", "UNH", "AXP", "CRM",
        "V", "AMGN", "IBM", "TRV", "HON", "MMM", "KO", "JNJ", "PG", "DIS"
    ],
    "DAX": [
        "SAP.DE", "SIE.DE", "ALV.DE", "DTE.DE", "MUV2.DE", "BAS.DE", "BMW.DE", "MBG.DE",
        "IFX.DE", "ADS.DE", "DB1.DE", "RWE.DE", "DHL.DE", "BAYN.DE", "HEN3.DE"
    ],
}
...
def macro_bonus(sector: str, industry: str, macro_name: str) -> float:
    ...
    for key, value in bonus_map.items():
        key_lower = key.lower()
        if key_lower in sector_lower or key_lower in industry_lower:
            bonus += value
...
def add_rows(rows: List[Dict[str, str]], symbols: List[str], source: str, suffix: str = "") -> None:
    for raw in symbols:
        sym = str(raw).strip()
        if not sym:
            continue
...
def get_universe(selected: Tuple[str, ...]) -> Tuple[pd.DataFrame, List[str]]:
    ...
    if "S&P 500" in selected:
        try:
            sp_tables = fetch_html_tables("https://en.wikipedia.org/wiki/List_of_S%26P_500_companies")
            sp = sp_tables[0]
    ...
    if "Dow Jones" in selected:
        try:
            dow_tables = fetch_html_tables("https://en.wikipedia.org/wiki/Dow_Jones_Industrial_Average")
            dow = None
    ...
    if universe_df.empty:
        raise RuntimeError("Kein Universum geladen.")
...
def compute_price_features(pxv: pd.DataFrame) -> Dict[str, float]:
    ...
    ret_3m = float(last / close.iloc[-63] - 1.0) if len(close) > 63 else 0.0
    ret_6m = float(last / close.iloc[-126] - 1.0) if len(close) > 126 else 0.0

    high_52w = float(close.tail(252).max()) if len(close) >= 252 else float(close.max())
    dist_to_52w_high = float(last / high_52w - 1.0) if high_52w > 0 else 0.0

    hist_vol_30d = float(close.pct_change().tail(30).std() * np.sqrt(TRADING_DAYS))
    avg_dollar_volume_m = float((close.tail(20) * volume.tail(20)).mean() / 1e6)
...
def scan_best_put_for_wheel(tk: yf.Ticker, snap: UnderlyingSnapshot) -> Optional[Dict]:
    ...
    for expiry in expiries:
        ...
        for _, row in puts.iterrows():
            ...
            sigma = iv if iv and iv > 0 else max(snap.hist_vol_30d, 0.15)
            ...
            iv_hv_ratio = (iv / snap.hist_vol_30d) if snap.hist_vol_30d > 0 and iv and iv > 0 else 1.0
            ...
            candidates.append(
                {
                    ...
                    "Sector": snap.sector,
                    "Score": round(score, 1),
                    "Warum": "CSP: Rendite + Liquidität + Delta + Buffer + IV/HV",
                }
            )
...
def score_leverage_candidate(snap: UnderlyingSnapshot) -> Optional[Dict]:
    ...
    if valuation_anchor is None:
        valuation_anchor = 40.0
...
if selected_tuple:
    universe_df, universe_errors = get_universe(selected_tuple)
else:
    universe_df = pd.DataFrame(columns=["Ticker", "Source"])
    universe_errors = []
...
if st.button("🔥 Scan starten", type="primary"):
    ...
    with col1:
        ...
        if wheel_rows:
            wheel_df = pd.DataFrame(wheel_rows).sort_values(
                ["Score", "AnnualizedYield%"],
                ascending=[False, False],
            )
    ...
    with col2:
        ...
        if leverage_rows:
            lev_df = pd.DataFrame(leverage_rows).sort_values(
                ["Score", "Ret3M%"],
                ascending=[False, False],
            )
    ...
    if underlyings:
        ...
        st.dataframe(
            base_df.sort_values(["DollarVolM", "MarketCapBn"], ascending=[False, False]),
...
st.info(
    "Hinweis: Das ist ein Screening-Tool. Kostenlose Daten sind brauchbar, aber nicht perfekt. "
